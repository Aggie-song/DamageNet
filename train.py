...
if __name__ == "__main__":
    ...
    #------------------------------------------------------------------#
    num_workers = min(os.cpu_count(), 2)  # 根据CPU核心数调整
    ...
    from nets.CNN_Transformer.cnn_trans import TransFuse_S
    ...
    #   读取数据集对应的txt
    #---------------------------#
    with open(os.path.join(dataset_txt_path, "train2.txt"),"r") as f:
        train_lines = f.readlines()
    with open(os.path.join(dataset_txt_path, "val2.txt"),"r") as f:
    ...
    if True:
        ...
        def debug_collate(batch):
            collated = unet_dataset_collate(batch)
            print(f"Collated batch structure: {len(collated)} elements")  # 调试输出
            print([type(x) for x in collated])  # 显示元素类型
            return collated
        ...
        #----------------------#
        if local_rank == 0:
            eval_callback   = EvalCallback(model, input_shape, num_classes, val_lines, log_dir, Cuda, \
